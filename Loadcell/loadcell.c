...
unsigned int Get_Maopi(){ 
    HX711_Buffer = HX711_Read(); 
    Weight_Maopi = HX711_Buffer/120; 
    return Weight_Maopi; 
} 
void main(void)
{
...
lcd_init(16);
lcd_gotoxy(0,0); 
lcd_puts("Timbangan 1 kg"); 
lcd_gotoxy(0,1); 
lcd_puts("Fisika Instrumen"); 
delay_ms(1500); 
lcd_clear(); 
lcd_gotoxy(0,0); 
lcd_puts("Baca ADC"); 
...
while (1)
      ...
      // Place your code here
        ...
        Weight = Get_Maopi(); 
        lcd_gotoxy(0,0); 
        sprintf(ch,"ADC : %d",Weight); 
        lcd_puts(ch); 
        Weight_Shiwu =(unsigned int)(((float)Weight*0.061)-533.5);
        lcd_gotoxy(0,1); 
        sprintf(text,"Berat : %d g",Weight_Shiwu); 
        lcd_puts(text); 
        delay_ms(1000); 
      }
}
